...
const (
	// svcName is the name of xzcd service.
	svcName = "xzcdsvc"
...
// logServiceStartOfDay logs information about xzcd when the main server has
...
// xzcdService houses the main service handler which handles all service
// updates and launching xzcdMain.
type xzcdService struct{}
...
// long-running xzcdMain (which is the real meat of xzcd), handles service
// change requests, and notifies the service control manager of changes.
func (s *xzcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start xzcdMain in a separate goroutine so the service can start
...
// installService attempts to install the xzcd service.  Typically this should
...
// removeService attempts to uninstall the xzcd service.  Typically this should
...
// startService attempts to start the xzcd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &xzcdService{})
	if err != nil {
