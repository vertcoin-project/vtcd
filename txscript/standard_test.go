...
// newAddressPubKey returns a new vtcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) vtcutil.Address {
	addr, err := vtcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new vtcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) vtcutil.Address {
	addr, err := vtcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new vtcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) vtcutil.Address {
	addr, err := vtcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the vtcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the vtcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := vtcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := vtcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := vtcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := vtcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := vtcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*vtcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*vtcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*vtcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*vtcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*vtcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
